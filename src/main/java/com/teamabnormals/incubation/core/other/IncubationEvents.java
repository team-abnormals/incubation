...
import com.teamabnormals.incubation.common.entity.goal.LayEggInNestGoal;
...
public class IncubationEvents {
	...
	public static void onBiomeLoad(BiomeLoadingEvent event) {
		...
		if (ModList.get().isLoaded("environmental") && !DataUtil.matchesKeys(biome, Biomes.FROZEN_RIVER) && (event.getCategory() == BiomeCategory.SWAMP || event.getCategory() == BiomeCategory.RIVER)) {
			generation.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, IncubationPlacedFeatures.NEST_DUCK);
		} else if (ModList.get().isLoaded("autumnity") && biome.toString().contains("maple") || biome.toString().contains("pumpkin_fields")) {
