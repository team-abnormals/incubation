...
public class BirdNestFeature extends Feature<NestConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NestConfiguration> context) {
		...
		if (level.isEmptyBlock(blockpos) && level.getBlockState(blockpos.below()).is(Blocks.GRASS_BLOCK)) {
			...
			EntityType<?> entityType = BuiltInRegistries.ENTITY_TYPE.get(context.config().entityType);
			if (entityType.create(level.getLevel()) instanceof Mob) {
				for (int i = 0; i < 4; ++i) {
					...
					if (level.noCollision(entityType.getSpawnAABB(posX, posY, posZ))) {
						...
						if (entity != null && SpawnPlacements.checkSpawnRules(entityType, level, MobSpawnType.STRUCTURE, BlockPos.containing(posX, posY, posZ), level.getRandom())) {
							...
							entity.finalizeSpawn(level, level.getCurrentDifficultyAt(entity.blockPosition()), MobSpawnType.STRUCTURE, null);
